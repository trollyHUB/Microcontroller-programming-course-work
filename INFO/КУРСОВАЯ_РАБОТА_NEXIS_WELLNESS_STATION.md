...
Выполнил(а): обучающийся(аяся) группы **ВТиПо-33**
...
Студенту: **Манасов Төлеген Айдарұлы**
...
**Срок сдачи работы студентом:** 13 мая 2026 г.
...
1. Провести теоретический анализ предметной области: выполнить обзор и сравнительный анализ существующих IoT-решений для мониторинга рабочей среды, обосновать выбор аппаратной платформы (микроконтроллер ESP32) и датчиков, исследовать применимые протоколы передачи данных и технологический стек веб-дашборда.

2. Спроектировать и реализовать систему NEXIS Wellness Station: разработать принципиальную схему устройства и собрать аппаратный прототип; разработать прошивку ESP32 (C++/Arduino Framework) с модулями опроса датчиков, управления дисплеем, Pomodoro-таймером, системой алертов и механизмом аппаратных прерываний (ISR); реализовать HTTP-интеграцию ESP32 с сервером; разработать веб-дашборд на FastAPI (Python) с SQLite-базой данных и браузерным SPA-интерфейсом.

3. Провести всестороннее тестирование системы (модульное, интеграционное, функциональное, нагрузочное) и разработать экономическое обоснование проекта.
...
**Научная база.** Исследования когнитивной психологии [5, 6] подтверждают, что принудительное чередование периодов концентрации и отдыха повышает общую производительность на 35–40% по сравнению с непрерывной работой, снижает уровень стресса и уменьшает вероятность профессионального выгорания.
...
**FR-6: Интегральная метрика качества рабочей среды (Wellness Index)**

- FR-6.1: Расчёт Wellness Index (WI) — интегрального показателя от 0 до 100 на основе взвешенной суммы нормализованных компонент по 5 параметрам: CO₂, температура, освещённость, влажность, шум
- FR-6.2: Пять уровней интерпретации WI: Отличный (85–100), Хороший (70–84), Удовлетворительный (55–69), Плохой (40–54), Критический (0–39)
- FR-6.3: Отображение WI в виде круговой диаграммы на главной странице веб-дашборда с рекомендацией пользователю
...
Полная принципиальная схема устройства с обозначением всех компонентов (DD1–DA7, R1–R5, VD1–VD3, HA1) представлена в Приложении Ж.
...
Рисунок 2.3 — Распиновка ESP32 DevKit V1 (30-pin) с указанием подключений NEXIS Wellness Station
...
Подробные блок-схемы алгоритмов прошивки (главный цикл, обработка прерываний, диаграмма состояний Pomodoro, диаграмма последовательности передачи данных) приведены в Приложении Е.
...
Рисунок 2.4 — Сравнение polling и interrupt-driven подходов
...
```
WI = w₁·T_score + w₂·CO2_score + w₃·Light_score + w₄·H_score + w₅·Noise_score   (2.1)
```

Где веса отражают относительный вклад параметра в когнитивную работоспособность согласно данным COGfx Study [4]:
...
```
score = clamp((x_max − x) / (x_max − x_min), 0.0, 1.0)                          (2.2)
```

Для CO₂ кусочно-линейная нормализация с двумя сегментами:

```
При x ∈ [600, 800] ppm:   CO2_score = 1.0 − (CO2 − 600) / (800 − 600) × 0.5    (2.3)
При x ∈ [800, 1200] ppm:  CO2_score = 0.5 − (CO2 − 800) / (1200 − 800) × 0.5   (2.4)
...
В ходе выполнения курсовой работы поставленная цель достигнута в полном объёме: разработана и реализована интеллектуальная станция мониторинга здоровья и продуктивности на рабочем месте **NEXIS Wellness Station** — полноценная IoT-система, объединяющая непрерывный мониторинг пяти параметров микроклимата, инструмент управления продуктивностью по технике Pomodoro, трёхуровневую систему алертов и современный веб-дашборд с визуализацией данных в реальном времени.

Все три задачи, сформулированные во введении, выполнены в полном объёме.

**Задача 1 — Теоретический анализ предметной области.** Проведён обзор и сравнительный анализ существующих решений в области IoT-мониторинга рабочей среды. Установлено, что коммерческие аналоги (Xiaomi Mi Air Monitor — 150 000 ₸, Netatmo Weather Station — 200 000 ₸) направлены исключительно на пассивный мониторинг параметров, не интегрируя инструменты управления продуктивностью. Обосновано, что микроконтроллер ESP32 DevKit V1 (двухъядерный Xtensa LX6, 240 МГц, 520 КБ ОЗУ, встроенный Wi-Fi) является оптимальной платформой для данной задачи: ни один из альтернативных микроконтроллеров (Arduino Uno, ESP8266, STM32F103) не обеспечивает одновременно необходимой производительности, беспроводной связи и количества GPIO. Исследованы интерфейсы I2C, UART, ADC, GPIO и Wi-Fi/HTTP, изучены стандарты ГОСТ 30494-2011 и ASHRAE 62.1-2019, обосновано применение техники Pomodoro с опорой на данные когнитивной психологии [5, 6]. Сформирован технологический стек: FastAPI + python-socketio + SQLite + Chart.js + JavaScript SPA.

**Задача 2 — Проектирование и реализация системы.** Разработана принципиальная схема устройства с позиционными обозначениями по ГОСТ 2.710-81 (DD1, DA1–DA7, R1–R5, VD1–VD3, HA1, SB1–SB2). Собран аппаратный прототип на макетной плате с 7 датчиками по четырём интерфейсам: I2C (BME280, BH1750, LCD 1602), UART (ZM106-VOC), 1-Wire (DHT11), ADC (KY-037, MQ-135) и цифровому GPIO (PIR). Разработана прошивка ESP32 (~750 строк C++) с модульной структурой: параллельный опрос 7 датчиков, 5-экранный LCD-интерфейс с автосменой каждые 3 с, Pomodoro-таймер с обратным отсчётом, трёхуровневая система алертов (OK/WARN/DANGER) с RGB LED и зуммером. Ключевая техническая особенность — механизм аппаратных прерываний GPIO: кнопки MODE и OK обрабатываются по FALLING-edge, датчик движения PIR — по RISING-edge; применён паттерн «volatile-флаг + обработка в loop()» с debounce-защитой 200 мс и размещением ISR в IRAM. Реализована HTTP-интеграция: ESP32 передаёт JSON-пакеты каждые 10 с на FastAPI-сервер с автоматическим переподключением при разрыве Wi-Fi. Разработан веб-дашборд — сервер FastAPI + SQLite + Socket.IO (10 API-эндпоинтов) и браузерный SPA на 13 JavaScript-модулях (~5 400 строк, 10 страниц): система достижений (19 условий), корреляционный анализ Pearson, тепловая карта продуктивности 7×24, PDF-экспорт, Web Audio API. Разработана интегральная метрика Wellness Index (WI) — взвешенная оценка качества рабочей среды по 5 параметрам (формула 2.1) с весами, обоснованными данными COGfx Study [4]: WI = 0.30·CO₂ + 0.25·T + 0.20·Light + 0.15·H + 0.10·Noise.

**Задача 3 — Тестирование и экономическое обоснование.** Проведено всестороннее тестирование из 17 тест-кейсов по четырём категориям: все 17 пройдены успешно (100%). Нагрузочный тест длительностью 7 суток подтвердил стабильность работы: 0 перезагрузок ESP32 и сервера, потеря пакетов 0.02% (12 из 60 480), отсутствие утечек памяти, рост БД 8.4 МБ за неделю. Показано, что система обеспечивает пятикратный запас по потребляемому току (415 мА при лимите USB 2000 мА), использует 40% ОЗУ ESP32 и 74.4% Flash. Себестоимость аппаратной части составила 15 700 ₸ — в 10–13 раз ниже коммерческих аналогов при более широком функционале.

**Научная и практическая значимость работы.** Разработанная система является первым известным открытым решением, объединяющим в рамках одного устройства ценой до 20 000 ₸ следующие возможности: мониторинг 5 параметров микроклимата (температура, влажность, давление, CO₂/VOC, освещённость, шум, присутствие), активный Pomodoro-таймер с логированием, трёхуровневую систему алертов с аппаратной индикацией, интегральный Wellness Index, веб-дашборд реального времени с аналитикой и экспортом данных. Устройство обладает реальной ценностью для следующих групп пользователей: студентов и школьников — для организации эффективных учебных сессий с контролем условий среды; программистов и фрилансеров — для мониторинга условий труда и управления временем; образовательных учреждений — для контроля микроклимата в аудиториях; малого бизнеса — как доступная альтернатива профессиональным системам мониторинга. Открытый исходный код проекта позволяет свободно адаптировать и расширять систему под конкретные нужды, что принципиально отличает NEXIS Wellness Station от проприетарных коммерческих решений.

**Новизна разработки** заключается в уникальном сочетании: (1) интеграция экологического мониторинга с инструментами тайм-менеджмента в одном устройстве; (2) применение интегральной метрики Wellness Index с весами, научно обоснованными COGfx Study; (3) PIR-детекция присутствия для автопаузы Pomodoro при отсутствии пользователя; (4) демо-режим для демонстрации работы без физического устройства; (5) полная трассировка 25 требований (RTM) с покрытием 100%.

**Перспективы развития проекта.** Архитектура системы была изначально спроектирована с учётом будущего расширения — свободные ресурсы ESP32 (312 КБ ОЗУ, 11 GPIO, 1 UART, 1 МБ Flash) оставляют достаточный запас. Приоритетные направления:

1. **Офлайн-буферизация данных (SD-карта, SPI)** — при потере Wi-Fi данные записываются на SD-карту (CS=GPIO5, MOSI=GPIO23, MISO=GPIO19, SCK=GPIO18), при восстановлении соединения синхронизируются с сервером. Это устранит единственную текущую слабость — потерю ~12 пакетов при сетевых сбоях за 7 суток.
2. **Замена ZM106-VOC на NDIR-датчик SCD41 (Sensirion)** — переход на истинное измерение CO₂ фотоакустическим методом с точностью ±40 ppm вместо текущих ±15%; стоимость ~4 000 ₸.
3. **MQTT-протокол вместо HTTP** — снижение трафика в 3–5 раз и переход к pub/sub-архитектуре для масштабирования на сотни устройств (интеграция с Home Assistant, Node-RED).
4. **Замена LCD 1602 на OLED SSD1306** — разрешение 128×64 пикселей, графики прямо на устройстве, потребление 0.04 Вт против 0.1 Вт у LCD, полное отсутствие подсветки.
5. **RTC DS3231** — автономный модуль часов реального времени с батарейным питанием для корректных меток времени без подключения к интернету; особенно важно для режима SD-карты.
6. **Мобильное приложение (Flutter/React Native)** — кросс-платформенный интерфейс для iOS и Android с push-уведомлениями при превышении порогов алертов и интеграцией с Telegram Bot API.
7. **Машинное обучение** — накопленные данные за несколько недель позволяют обучить простую модель (Random Forest или LSTM) для предсказания снижения продуктивности по паттернам CO₂, температуры и времени суток.
8. **Серийное производство** — при тираже 100 единиц себестоимость снижается до 10 500 ₸ за счёт оптовых закупок, что при розничной цене 29 990 ₸ обеспечивает окупаемость уже после продажи 19 устройств.

**Освоенные компетенции.** В процессе работы над проектом приобретены следующие навыки, соответствующие целям дисциплины PM3304: программирование микроконтроллеров на C++/Arduino Framework; работа с интерфейсами I2C, UART, ADC, GPIO и механизмами аппаратных прерываний; отладка прошивок с помощью Serial Monitor и осциллографа; разработка IoT-систем с беспроводной передачей данных; проектирование REST API и WebSocket-серверов на Python; построение браузерных SPA-приложений; нагрузочное тестирование встраиваемых систем; составление технической документации и принципиальных схем.

Таким образом, в рамках курсовой работы не только достигнуты все поставленные учебные цели, но и создан реально работающий открытый продукт, способный принести практическую пользу людям умственного труда и послужить основой для дальнейших исследований в области IoT-мониторинга условий труда.
...
23. Statista Research Department. Number of Internet of Things (IoT) connected devices worldwide from 2019 to 2030. — Hamburg: Statista, 2023. — URL: https://www.statista.com/statistics/1183457/iot-connected-devices-worldwide/ (дата обращения: 10.03.2026).

24. Majumder S., Mondal T., Deen M.J. Wearable Sensors for Remote Health Monitoring // Sensors. — 2023. — Vol. 17, №1. — Article 130. — DOI: 10.3390/s17010130.

25. Tanvir R., Mahbub M. A Smart Occupancy Monitoring System Based on CO₂ and Temperature Sensing for Energy-Efficient Buildings // Sensors. — 2023. — Vol. 23, №4. — Article 1892. — DOI: 10.3390/s23041892.

26. Liang X., Wang Y., Chen Z. ESP32-Based IoT Gateway for Multi-Sensor Environmental Data Acquisition and Real-Time Web Visualization // Applied Sciences. — 2024. — Vol. 14, №3. — Article 1045.
...
## ПРИЛОЖЕНИЕ В — Листинг основных функций прошивки ESP32 (main.cpp)

> **Примечание:** В данном приложении представлены основные функции прошивки.
> Полный исходный код прошивки находится в файле `esp32/src/main.cpp` репозитория проекта.
...
## ПРИЛОЖЕНИЕ Ж — Принципиальная схема подключения компонентов NEXIS Wellness Station
...
>
> Принципиальная схема выполнена в draw.io и содержит все компоненты системы с указанием
> интерфейсов (I2C, UART, ADC, GPIO), номеров выводов, уровней питания (3.3 В / 5 В)
> и номиналов защитных элементов.

### Ж.1 Функциональная схема подключения (упрощённый вариант)

Ниже приведена упрощённая функциональная схема, отражающая логику подключения компонентов — интерфейсы, номера GPIO и шины питания — без детализации защитных элементов. Данный вариант удобен для быстрого ориентирования при монтаже прототипа.
...
Рисунок Ж.1 — Упрощённая функциональная схема подключения компонентов NEXIS Wellness Station

---

### Ж.2 Принципиальная схема подключения с ГОСТ-обозначениями (полный вариант)

Данный вариант является доработанной версией схемы, содержащей полные позиционные обозначения элементов в соответствии с ГОСТ 2.710-81 (УГО для электронных компонентов), номиналы всех пассивных элементов и указание режимов работы портов GPIO.

Условные обозначения: DD1 — ESP32 DevKit V1; DA1 — BME280; DA2 — BH1750; DA3 — ZM106-VOC; DA4 — PIR HC-SR501; DA5 — DHT11; DA6 — MQ-135; DA7 — LCD 1602; VD1–VD3 — RGB LED; R1–R3 — резисторы 220 Ом (LED); R4 — резистор 100 Ом (зуммер); R5 — резистор 10 кОм (подтяжка DHT11); HA1 — зуммер пассивный; SB1, SB2 — кнопки тактовые.

```
                            DD1 — ESP32 DevKit V1
                     ┌──────────────────────────────┐
  +3.3V ─────────────┤3V3                       GND ├─── GND
  +5V (USB) ─────────┤VIN                      GPIO4├─── R5(10кОм)─── DA5(DHT11)
                     │                         GPIO36├─── DA6(MQ-135 AOUT)
  [I2C_SDA]─────────►┤GPIO21(SDA)             GPIO39├─── DA7(KY-037 AOUT)
  [I2C_SCL]─────────►┤GPIO22(SCL)             GPIO27├─── DA4(PIR OUT)
                     │                              │
  DA3 TX ───────────►┤GPIO16(RX2)             GPIO32├─── SB1(BTN MODE)
  DA3 RX ◄───────────┤GPIO17(TX2)             GPIO33├─── SB2(BTN OK)
                     │                              │
  R1─VD1(R)─GND ◄───┤GPIO13                  GPIO25├─── R4─HA1(Buzzer)
  R2─VD2(G)─GND ◄───┤GPIO12                        │
  R3─VD3(B)─GND ◄───┤GPIO14                        │
                     └──────────────────────────────┘

I2C ШИНА (GPIO21=SDA, GPIO22=SCL):
  ├── DA1 BME280  (адрес 0x76, питание 3.3V)
  ├── DA2 BH1750  (адрес 0x23, питание 3.3V)
  └── DA7 LCD1602 + PCF8574 (адрес 0x27, питание 5V!)

Питание компонентов:
  3.3V (от LDO ESP32): DA1-BME280, DA2-BH1750, DA5-DHT11,
                        DA6-KY-037, VD1-VD3 (RGB LED), HA1, SB1, SB2
  5V (от USB/VIN):     DA7-LCD1602, DA3-ZM106-VOC, DA4-PIR HC-SR501
```

Рисунок Ж.2 — Принципиальная схема подключения компонентов NEXIS Wellness Station (с ГОСТ-обозначениями)

### Ж.3 Схема защитных элементов

```
SB1(BTN MODE, GPIO32) и SB2(BTN OK, GPIO33):
  GPIO ──── [кнопка] ──── GND
  Режим: INPUT_PULLUP (встроенный подтягивающий резистор ~45 кОм)

RGB LED (VD1, VD2, VD3) + токоограничивающие резисторы:
  GPIO13 ──[R1=220 Ом]── VD1(LED R, Anode) ── GND(Cathode)
  GPIO12 ──[R2=220 Ом]── VD2(LED G, Anode) ── GND(Cathode)
  GPIO14 ──[R3=220 Ом]── VD3(LED B, Anode) ── GND(Cathode)
  Ток через LED: I = (3.3 − 2.0) / 220 ≈ 6 мА (в пределах нормы ESP32)

HA1 Зуммер пассивный:
  GPIO25 ──[R4=100 Ом]── HA1(+) ── GND
  ШИМ-сигнал tone() для генерации тонов (функция Arduino)

DA5 DHT11 — подтяжка линии данных:
  VCC ──[R5=10 кОм]── DATA ──── GPIO4
  DATA: однопроводной протокол 1-Wire

DA3 ZM106-VOC (UART, уровни 5В):
  VCC=5V, GND
  TX датчика ──── GPIO16 (RX2 ESP32, 3.3V tolerant)
  RX датчика ──── GPIO17 (TX2 ESP32) ─── [делитель напряжения или согласователь]
...
### Ж.4 Перечень элементов принципиальной схемы
...
Таблица Ж.1 — Перечень позиционных элементов принципиальной схемы NEXIS Wellness Station

---

*Дата выполнения работы: «13» мая 2026 г.*

*Подпись студента: _______________________  Манасов Т.А.*

*Подпись руководителя: _______________________  Байдельдинов М.У.*
